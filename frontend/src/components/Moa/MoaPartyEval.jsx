...
const MoaPartyEval = (props) => {
  ...
  const evalList = partyEvalInfo?.partyPlayers.map((player)=>{
  ...
  useEffect(()=> {
    setEvalInfo(partyEvalInfo?.partyPlayers.map((player)=>{
      if (player.userId !== userId){
        return {
  ...
  const onSubmit = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        evalInfo.map((evaluation,idx)=>{
          postPartyEval(evaluation).then((res)=> console.log(res)).catch((err)=>console.log(err))
        })
  ...
  return (
    // modal
    <div id="following-modal" className={`${(showEvalModal ? "bg-black bg-opacity-50":"hidden")} overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full`}>
      ...
      <div className="relative bg-white rounded-lg shadow w-full">
        ...
        <div className="flex justify-between items-center p-5 rounded-t border-b">
            <div className=" font-medium text-gray-800">
              <div className='text-sm font-semibold mb-0.5'>
                [{partyEvalInfo.gameName}]
              </div>
              <div className='text-sm text-baseline'>
              <span className='text-base font-bold mr-2'>{partyEvalInfo.partyTitle}</span> <span>파티원 평가</span>
              </div>
            </div>
        ...
        <div className="px-6 py-4 space-y-4 h-[400px] overflow-y-auto ">
          <div className='text-[13px]'>
            <div>
              모아러들의 더 좋은 경험을 위해 <span className='text-base text-moa-purple font-bold'>{userId}</span>님의 모아 경험을 평가해주세요.
            </div>
            <div>
              점수를 입력하지 않고 제출하게 되면 0점으로 평가됩니다.
            </div>
            <div className='mb-4'>
              꼭 점수를 선택하고 제출해주세요!
            </div>
        ...
        <div className='flex justify-end p-3'>
          <button className='px-2.5 py-1.5 bg-moa-purple rounded text-white hover:bg-moa-purple-dark hover:cursor-pointer drop-shadow-md'
                  onClick={onSubmit}>평가 제출하기</button>
        </div>
      </div>
    </div>
  </div>
