        ...
        <div className='grid grid-cols-5 gap-2'>
          <div className='col-span-2 py-2 px-3'>
            <div className='w-full h-full flex flex-col justify-between items-center'>
              {today.length ?<MainGameComponent game={today[Math.floor(Math.random() * 4)]} type="today"/>:''}
              {hots.length ?<MainGameComponent game={frees[Math.floor(Math.random() * 10)]} type="hot"/>:''}
              {bests.length ?<MainGameComponent game={bests[Math.floor(Math.random() * 5)]} type="best"/>:''}
            </div>
    
          </div>
          <div className='col-span-3'>
            {today.length ? <img src={today[4].gameImgpath} alt="" className='rounded w-full drop-shadow-md hover:cursor-pointer' onClick={onClickToday}/> : ""}
          </div>
        </div>
